...
<Sidebar
	...
	{toggle}
	bind:editWork
	bind:open={menubutton}
	{form}
	bind:bearbeiten={bearbeitenReactiv}
/>

<div class="appWrap">
	<div class="workDisplay">
		{#if !editWork}
			<button on:click={() => (menubutton = !menubutton)} style="border:solid 1px red; "
				>Menü</button
			>
			{#if menubutton}
				This can be the trigger for the Menü
			{/if}
		{/if}
		<!-- <div class="todo red">
			Hier startet dann die scrollseite mit allen arbeiten
			<h1>
				TODO : Der LocatorUpdater beim Edit Button muss noch bearbeitet werden, da die Auswahl beim
				LocatorUpdater nicht korrekt angezeigt wird
			</h1>
		</div> -->
